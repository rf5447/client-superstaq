...
os.makedirs("fswap_ibm", exist_ok=True)
...
for nq in [4, 5, 7, 11, 12, 13, 16]:#, 20]:
    # QAOA Fermionic Swap Proxy returns a single circuit
    ...
    try:
        ...
        for backend_name in backend_names:
            print(f"Transpiling nq={nq} for {backend_name}")
        ...
        print(f"Error preparing nq={nq}: {e}")
...
for backend_name in backend_names:
    ...
    for i, pub_result in enumerate(result):
        ...
        # Filename updated to reflect fswap_ibm directory and nq
        filename = f"fswap_ibm/fswap_q{m['nq']}_{backend_name}.json"
