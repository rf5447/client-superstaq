...
out_dir = "phasecode_circuit_pngs"
...
for nq in [3, 4, 5, 7, 9]:
    for nr in [2, 3, 4]: #, 5, 7, 9]:
        ...
        phasecode = supermarq.phase_code.PhaseCode(nq, nr, phase_state)
        phasecode_circuit = phasecode.circuit()

        out_path = os.path.join(out_dir, f"phasecode_{nq}q_{nr}r.png")
        save_circuit_png(phasecode_circuit, out_path)
